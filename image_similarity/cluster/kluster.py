...
def getSimilarityAndDistance(out1, out2):
    '''
    Getting similarity and distance
    '''
...
def post_process_dummy(img1, img2):
    '''
    To getting similarity and distance
    '''
    # Determine and set context
    if len(mx.test_utils.list_gpus()) == 0:
        ctx = mx.cpu()
    ...
    # Configure face detector
    det_threshold = [0.6, 0.7, 0.8]
    mtcnn_path = os.path.join(
            os.path.dirname('__file__'), 'mtcnn-model')
...
img1 = cv2.imread('gambar/a1.png')

# out1, out2 = post_process_dummy(img1, img2)
# print(out1, out2)
# print(type(out1))
# print(len(out1))
...
for i in range(len(path_picture)):
    if path_picture[i].endswith('.png'):
        print(i)
        img_path = cv2.imread('gambar/' + path_picture[i])
        print(img_path)
...
kmeans.fit(vector_group)

#filename = 'face_cluster_model.sav'
#pickle.dump(model, open(filename, 'wb'))
#
## load the model from disk
#loaded_model = pickle.load(open(filename, 'rb'))
#
#kluster = loaded_model.cluster_centers_
#print("kluster============================")
#print(kluster)
#print(len(kluster))
#print(type(kluster))
